...
pub struct AppStateInner {
    pub network: Network,
    pub config: Config,
    pub rpc_client: xnt_rpc::AuthenticatedClient,
...
impl AppState {
    pub async fn init() -> Result<Self, anyhow::Error> {
        let rpc_client = xnt_rpc::gen_authenticated_rpc_client()
            .await
        ...
        let genesis_digest = rpc_client
            ...
            .await
            .with_context(|| "Failed calling xnt-core api: block_digest")?
            .with_context(|| "Failed calling xnt-core api method: block_digest")?
            .with_context(|| "xnt-core failed to provide a genesis block")?;
    ...
    /// to re-establish connection to the XNT RPC server.
    ///
    ...
    /// mutability occurs.
    pub fn set_rpc_client(&self, rpc_client: xnt_rpc::AuthenticatedClient) {
